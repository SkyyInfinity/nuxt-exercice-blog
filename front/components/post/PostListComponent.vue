<template>
    <div class="posts">
        <ul class="posts-list grid gap-8 lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1">
            <li v-for="(post, index) in posts" :key="post.id" :data-index="index" class="post bg-white rounded-lg overflow-hidden border border-slate-200 hover:scale-[1.02] transition will-change-transform">
